...
function makeEvent() {
  ...
  if (type === 'bayes') {
    ...
    if (sub === 'update') {
      const t = pick(TICKERS), from = r(0.44,0.65), to = r(0.58,0.86)
      return { id, time, type, tag:'posterior update', isNew:true,
        headline:`${t} belief revised — ${to>from?'bullish':'bearish'}`,
    ...
    return { id, time, type, tag:'pair trade', isNew:true,
      ...
      detail:`z-score ${r(1.8,2.9)} · cointegração p < 0.01 · long ${a} / short ${b} opened` }
  }

  if (type === 'release') {
    if (Math.random() > 0.4) {
      const t = pick(TICKERS), sent = r(-0.3,0.6), surp = r(-0.25,0.5)
  ...
  if (type === 'macro') {
    ...
    if (sub === 'focus') {
      const to = r(3.8,4.6,1), from = +(to+r(-0.3,0.3,1)).toFixed(1)
      return { id, time, type, tag:'macro', isNew:true,
        headline:'Focus — inflação expectation revised',
        detail:`IPCA 2026: ${from} → ${to} · risk-on regime confidence ${pick(['+2pp','-1pp','+3pp','-2pp'])}` }
    }
    if (sub === 'fiscal')
      return { id, time, type, tag:'macro', isNew:true,
  ...
  const hawkish = r(0.54,0.84), shift = r(-0.22,0.22)
...
export function useEventStream(initialEvents) {
  // FIX: start empty — populated by the effect below when data arrives
  ...
  useEffect(() => {
    if (!initialEvents?.length || seeded.current) return
    seeded.current = true
    setEvents(initialEvents.map((e, i) => ({ ...e, id: i, isNew: false })))
  }, [initialEvents])
  ...
  useEffect(() => {
    ...
    return () => { alive.current = false; clearTimeout(timeoutRef.current); clearTimeout(clearRef.current) }
  }, [])
